...
func (r *ContainerRenderer) assignNames(ctx *GenContext) error {
	...
	for id := range ctx.services {
		...
		ctx.services[id].buildName = r.getterRegistry.BuildFunc(id)
		ctx.services[id].fieldName = r.getterRegistry.Field(id)
	}
...
func (r *ContainerRenderer) renderContainerStruct(b *bytes.Buffer, ctx *GenContext, hasParams bool) error {
	...
	fmt.Fprintf(b, "\tparamsResolver *parameters.Resolver\n")
	...
	fmt.Fprintf(b, "\t}\n")
	fmt.Fprintf(b, "\tc := &%s{\n", r.containerName)
	fmt.Fprintf(b, "\t\tparamsResolver: parameters.NewResolver(params, parameters.StandardCaster{}),\n")
