...
// decoratorBuildRenderer renders a decorator build function with inner parameter.
type decoratorBuildRenderer struct{}

func (r *decoratorBuildRenderer) buildSignature(ctx *genContext, svc *serviceDef) (string, string) {
...
func (r *decoratorBuildRenderer) render(b *bytes.Buffer, ctx *genContext, svc *serviceDef) error {
	signature, innerVar := r.buildSignature(ctx, svc)
	retType := ctx.imports.typeString(svc.typeName)
	returnsErr := buildNeedsErrorHandling(svc)
