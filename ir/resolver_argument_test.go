...
func TestTaggedElementTypeAssignable(t *testing.T) {
	...
	if _, err := r.resolve(ctx, "svc.one", 0, arg, types.NewSlice(types.Typ[types.Int])); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	emptyIface := types.NewInterfaceType(nil, nil)
	emptyIface.Complete()
	if _, err := r.resolve(ctx, "svc.two", 0, arg, types.NewSlice(emptyIface)); err != nil {
...
func TestTaggedElementTypeNotAssignable(t *testing.T) {
	...
	emptyIface := types.NewInterfaceType(nil, nil)
	emptyIface.Complete()
	if _, err := r.resolve(ctx, "svc.one", 0, arg, types.NewSlice(emptyIface)); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	_, err := r.resolve(ctx, "svc.two", 0, arg, types.NewSlice(types.Typ[types.Int]))
