...
func TestCollectPackagePaths(t *testing.T) {
	tests := []struct {
		name     string
		...
		{
			name: "type args from other packages",
			services: map[string]di.Service{
				"events": {
		...
		{
			name: "generic types in type field",
			services: map[string]di.Service{
				"pool": {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			cfg := &di.Config{Services: tt.services}

			refreshPackages(cfg)
			paths, _ := collectPackagePaths(cfg)

			expected := make(map[string]bool, len(tt.want))
			for p := range tt.want {
				expected[p] = true
			}
			for _, p := range paths {
